...
async def create_customer_portal_session(stripe_customer_id: str, origin_url: str) -> Dict[str, Any]:
    """
    ...
    try:
        session = stripe_lib.billing_portal.Session.create(
            ...
            return_url=f"{origin_url}/subscription/manage"
        )
...
async def create_checkout_session(request: CheckoutRequest, user_id: Optional[str] = Header(None, alias="X-User-ID")):
...
async def get_checkout_status(session_id: str, request: Request):
    ...
    try:
        ...
        if checkout_status.payment_status == "paid" and transaction.get("subscription_created") != True:
            ...
            existing = supabase.table("subscriptions").select("id").eq("session_id", session_id).execute()
            
            if existing.data and len(existing.data) > 0:
                # Update existing
                supabase.table("subscriptions").update(subscription).eq("session_id", session_id).execute()
            else:
                # Insert new
...
async def get_user_subscription(user_id: str) -> SubscriptionStatus:
    ...
    try:
        ...
        if trial_ends_at:
            trial_end_date = datetime.fromisoformat(trial_ends_at)
            is_trial = now < trial_end_date
...
async def stripe_webhook(request: Request):
    ...
    try:
        ...
        origin = request.headers.get("origin") or "https://your-domain.com"
        stripe_checkout = get_stripe_checkout(origin)
...
async def get_user_usage(user_id: str) -> Dict:
    ...
    assessments_response = supabase.table("assessment_sessions").select("id", count="exact").eq("user_id", user_id).eq("completed", True).gte("completed_at", month_start.isoformat()).execute()
    assessments_count = assessments_response.count or 0
    
    # Count AI insights used this month (from couple_results)
    insights_response = supabase.table("couple_results").select("id", count="exact").eq("user_id", user_id).gte("created_at", month_start.isoformat()).execute()
    ...
    return {
        ...
        "assessments_remaining": max(0, FREE_TIER_LIMITS["assessments_per_month"] - assessments_count),
